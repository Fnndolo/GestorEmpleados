...
| Al **firmar** el colaborador su orden de pago aparte (o al marcar el pago, si aquello falló) | `POST /api/horas/cierre { desde, hasta, documentos: [cédula] }` | Cierra allá el período de ESA persona: los tramos quedan congelados y marcados como pagados (`/pagadas` solo anota; `/cierre` es lo que bloquea). Idempotente (`yaCerrados`). Se guarda `PagoHorasExtra.asistenciaAnotadoEn`; si AsistencIA no respondió se avisa a TH y se reintenta al marcar el pago. Reabrir: `DELETE /api/horas/cierre` (no se llama desde aquí todavía). |
| Botón de prueba «lo que marcó hoy» (Nómina → Horas extra, por persona) | `GET /api/horas?desde=hoy&hasta=hoy` | Le manda al colaborador una notificación con sus tramos de hoy. AsistencIA **no expone las marcaciones** (entrada/salida) por API, solo los tramos con recargo: si hoy no generó ninguno, el aviso lo dice. Evento `asistencia_resumen_dia`. |
