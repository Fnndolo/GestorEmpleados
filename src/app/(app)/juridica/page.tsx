...
export default async function JuridicaPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  const [documentos, disciplinarios, denuncias, consultas] = await Promise.all([
    prisma.documentoLegal.findMany({ orderBy: { creadoEn: 'desc' }, take: 100 }),
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <JuridicaCliente
        ...
        documentos={documentos.map((d) => ({ id: d.id, categoria: d.categoria, titulo: d.titulo, vigenciaFin: d.vigenciaFin ? formatFechaISO(d.vigenciaFin) : null }))}
