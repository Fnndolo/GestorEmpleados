...
/** El contratista (OPS) crea y envía su propia cuenta de cobro. */
export const crearMiCuentaCobro = accion(
  ...
  async (d, usuario) => {
    ...
    if (!contrato) throw new ErrorNegocio('No tienes un contrato de prestación de servicios (OPS) activo.')

    const dup = await prisma.cuentaCobroOps.findUnique({ where: { contratoOpsId_periodo: { contratoOpsId: contrato.id, periodo: d.periodo } } })
    if (dup) throw new ErrorNegocio('Ya tienes una cuenta de cobro para ese periodo.')

    const total = await prisma.cuentaCobroOps.count({ where: { contratoOpsId: contrato.id } })
    const cuenta = await dbAuditado.cuentaCobroOps.create({
      data: {
        contratoOpsId: contrato.id, numero: `CC-${total + 1}`, periodo: d.periodo, concepto: d.concepto || null,
        valor: d.valor, fechaRadicacion: hoyBogota(), estado: 'RADICADA', creadaPorContratista: true,
      },
    })

    // Generar el PDF desde la plantilla elegida (o la de defecto)
    await generarPdfCuentaCobro(cuenta.id, d.plantillaId || null, usuario.id)
    ...
    await avisarPorRol(['Contador', 'Administrador', 'Subgerencia'], {
      ...
      mensaje: `${colab.nombres} ${colab.apellidos} radicó la cuenta de cobro ${cuenta.numero} (periodo ${d.periodo}). Recuerda verificar el pago de seguridad social antes de aprobar.`,
      enlace: `/contratos/ops/${contrato.id}`,
