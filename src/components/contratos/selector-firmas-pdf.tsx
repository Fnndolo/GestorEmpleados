...
/**
 * Las dos partes de cualquier contrato. Las claves son las de OPS y valen para
 * el laboral: `contratista` es quien se vincula (el trabajador) y `contratante`
 * la empresa. Solo cambia el rótulo que se muestra (`etiquetas`).
 */
export type Parte = 'contratista' | 'contratante'

const ETIQUETA_OPS: Record<Parte, string> = {
  contratista: 'Firma del contratista',
  contratante: 'Firma de la empresa',
}
/** Rótulos para un contrato de trabajo. */
export const ETIQUETAS_LABORAL: Record<Parte, string> = {
  contratista: 'Firma del trabajador',
  contratante: 'Firma del empleador',
}
...
export function SelectorFirmasPdf({
  ...
  /** Qué firmas se ubican. Si el PDF ya viene firmado por la empresa, solo la del contratista. */
  partes?: Parte[]
    ...
    <div className="space-y-2">
        ...
        {partes.map((parte) => {
          ...
          return (
            <div
              ...
              role="button"
              tabIndex={0}
              aria-label={`${etiquetas[parte]}: arrastra o usa las flechas para ubicarla`}
      ...
      <div className="flex flex-wrap gap-3 text-[11px] text-muted-foreground">
        {partes.map((parte) => (
          ...
          >
            {etiquetas[parte]}: página {valor[parte].pagina}
          </button>
        ))}
      </div>
    </div>
